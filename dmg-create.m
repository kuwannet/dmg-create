...
void printUsage();
void exitWithError( NSAutoreleasePool * pool, NSString * str, int nErr );
void exitWithErrorUsage( NSAutoreleasePool * pool, NSString * str, int nErr, BOOL bPrintUsage );
		...
		if ( [s hasPrefix:@"-"] )
		{
			...
			if ( [dictLangIDs valueForKey:s] != nil )
			{
				...
				if ( i < argc )
				{
					...
					exitWithErrorUsage( pool, @"Too few arguments, missing license file.", -1, YES );
...
//			}
			else if ( [s caseInsensitiveCompare:@"volname"] == NSOrderedSame )
			{
				...
				if ( i < argc )
				{
					...
					exitWithErrorUsage( pool, @"Too few arguments, missing volume name.", -1, YES );
			}
			else
			{
				exitWithErrorUsage( pool, [NSString stringWithFormat:@"Unrecognized option: %@", s], -1, YES );
			}
...
void printUsage()
{
	printf( "\nUsage: dmg-create <Source Folder> [-volname <Volume Name>] [-<Language> <license.txt>]\n\n" );
	printf( "   <Source Folder>           :  The root folder of the DMG, all contents will be copied to the DMG.\n" );
...
void exitWithError( NSAutoreleasePool * pool, NSString * str, int nErr )
{
	exitWithErrorUsage( pool, str, nErr, NO );
}
